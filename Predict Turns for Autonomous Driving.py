...
def show_curvatures(img, leftx, rightx, xmtr_per_pixel, ymtr_per_pixel):
    ...
    cv.putText(out_img, 'curvature of left line: {:.2f} m'.format(left_curvature),
                (50, 80), cv.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    cv.putText(out_img, 'curvature of right lane: {:.2f} m'.format(right_curvature),
...
def dist_from_center(img, left_fit, right_fit, xmtr_per_pixel, ymtr_per_pixel):
    ...
    if dist >= 0. :
        message = 'location: {:.2f} m right'.format(dist)
    else:
        message = 'location: {:.2f} m left'.format(abs(dist))
...
cap = cv.VideoCapture(r"C:\Users\chand\Downloads\challenge.mp4")
...
while cap.isOpened():
    ...
    result,out_img,left_fit,right_fit,prediction = Sliding_window(warped_img)
